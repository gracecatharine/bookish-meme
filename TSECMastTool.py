...
final_output_dir_default = "final_outputs"
reference_csv_default = "reference_copy.csv"

#final output filenames normalize the trailing "_{n}_months" window length to 12
normalized_months_value = 12
...
class PipelineConfig:
    ...
    final_output_dir: str = final_output_dir_default
...
#return path
    ...
    def final_path(self, filename):
        return os.path.join(self.final_output_dir, filename)
...
def ensure_final_output_dir(config):
    os.makedirs(config.final_output_dir, exist_ok=True)

def normalize_months_token(stem):
    return re.sub(r"\d+(?=_[Mm]onths?\b)", str(normalized_months_value), stem)
...
def load_reference(reference_csv):
    ...
    if missing:
        raise KeyError()
    df[reference_time_col] = pd.to_datetime(df[reference_time_col], errors = "coerce")
...
def discover_temperature_column(df, height):
    expected = f"Temp {height}m syn [°C]"
...
def run_stage3(config, targets=None, target_glob=None, speed_col=None):
    ...
    for mast in masts:
        ...
        if not full_speed_columns:
            raise ValueError(f"{mast.mast_id}: no Speedd {{height}}m syn [m/s] columns found")
        full_target_mean = {
...
def run_stage7(config, targets=None, target_glob=None):
    ...
    for mast in masts:
        ...
        final_path = write_final_timeseries(df_merged, mast.mast_id, config, mast.raw_path)
...
def write_final_timeseries(df_merged, mast_id, config, raw_input_path):
    ...
    #mast file headers - name/place the final output in its own folder, not in pipeline_outputs
    input_stem = Path(raw_input_path).stem
    normalized_stem = normalize_months_token(input_stem)
    ensure_final_output_dir(config)
    out_path = config.final_path(f"{normalized_stem}_scaled.csv")
...
def parse_args():
    ...
    parser.add_argument(
        "--final-output-dir",
        default=final_output_dir_default,
    )
