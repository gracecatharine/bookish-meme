...
"""
notes: this is the code that works with monarchwind format
...
def compute_global_overlap(config, masts):
    ...
    if global_start >= global_end:
        raise ValueError("No valid overlap across masts and reference")
...
def load_reference(reference_csv):
    ...
    #read every line
    with open(reference_csv, "r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            line = line.strip()
            if not line:
                continue
            #remove quotes
            line = line.strip('"')
            # split tab-delimited values into columns
    ...
    print(f"Reference rows loaded: {len(df):,}")
    print(f"Reference columns loaded: {len(df.columns)}")
    ...
    for col in df.columns:
        if col != reference_time_col:
            df[col] = pd.to_numeric(df[col], errors="coerce")

    #convert timestamp
    df[reference_time_col] = pd.to_datetime(
        df[reference_time_col],
        errors="coerce"
    )
    ...
    print(f"Final reference shape: {df.shape}")
    return df
...
#stage 4 temperature
def load_temperature_ratio_data(config):
    reference_path = config.path("ref_monthly_temp.csv")
    overlap_path = config.path("overlap_ref_monthly_temp.csv")
    long_term = pd.read_csv(reference_path)
    overlap = pd.read_csv(overlap_path)

    long_term["Temperature_k"] = long_term[temp_reference_col] + 273.15
    overlap["Temperature_k"] = overlap[temp_reference_col] + 273.15

    ratio_df = pd.DataFrame({
        "Month": long_term["Month"],
        "Reference_Temp_K": long_term["Temperature_k"],
        "Overlap_Temp_K": overlap["Temperature_k"],
    })
    ratio_df["Month"] = ratio_df["Month"].apply(normalize_month)
    if ratio_df["Month"].isna().any():
        raise ValueError()
    ratio_df["Ratio"] = ratio_df["Reference_Temp_K"] / ratio_df["Overlap_Temp_K"]
    return ratio_df
...
def apply_temperature_scaling(df_target, ratio_df, heights):
    ...
    for h in heights:
        original_temp = df_target[f"Temperature_k_{h}"].astype(float)
        scaled_temp = original_temp * monthly_scale
        df_target[f"scaled_temp_{h}K"] = scaled_temp
        df_target[f"scaled_temp_{h}C"] = scaled_temp - 273.15
...
def run_stage4(config, targets=None, target_glob=None):
    ensure_output_dir(config)
    masts = discover_masts(config, targets=targets, target_glob=target_glob)
    ratio_df = load_temperature_ratio_data(config)
    if config.write_diagnostics:
        ratio_df.to_csv(config.path("temperature_scaling_ratios.csv"), index=False)
    ...
    for mast in masts:
        ...
        for h in config.heights:
            source_col = discover_temperature_column(df_target, h)
            if source_col is None:
                raise KeyError(f"Missing temperature column for {h}m")
            df_target[f"Temperature_k_{h}"] = (
                df_target[source_col].astype(float) + 273.15
            )
        df_target = apply_temperature_scaling(df_target, ratio_df, config.heights)
        df_target = build_timestamp(df_target)
        df_target.to_csv(mast.temp_corrected_path, index=False)
        df_overlap = df_target[
            (df_target["_timestamp"] >= config.target_start) &
            (df_target["_timestamp"] <= config.target_end)
        ]
        if df_overlap.empty:
            raise ValueError(
                f"{mast.mast_id}"
                f"{config.target_start} and {config.target_end}."
            )
...
def compute_elevation(df, heights):
    ...
    for h in heights:
        ...
        if temp_col not in df.columns:
            raise KeyError(f"{temp_col}")

        if density_col not in df.columns:
            density_like = [c for c in df.columns if "density" in c.lower()]
            raise KeyError(
                f"{density_col}. "
                f"{density_like}"
            )
...
def recompute_density(df, heights, avg_elevation, P0=101325, R=287.05, g=-9.80665):
    ...
    for h in heights:
        ...
        if temp_col not in df.columns:
            raise KeyError(f"{temp_col}")
        if h not in avg_elevation:
            raise KeyError(f"missing averaged elevation for height {h}")
...
def compute_ti_columns(df, heights):
    ...
    for h in heights:
        ...
        if speed_col not in df.columns:
            raise KeyError(f"{speed_col}")
        if ti_pct_col not in df.columns:
            raise KeyError(f"{ti_pct_col}")
        ...
        if corrected_speed_col is None:
            raise KeyError()
...
def build_merged_frame(mast, config):
    ...
    if not os.path.exists(wind_path):
        raise FileNotFoundError(
            f"{mast.mast_id}, {wind_path}"
        )
    if not os.path.exists(temp_path):
        raise FileNotFoundError(
            f"{mast.mast_id}, {temp_path}"
        )
    if not os.path.exists(elevation_path):
        raise FileNotFoundError(
            f"{mast.mast_id}, {elevation_path}"
        )
    ...
    if not os.path.exists(best_window_path):
        raise FileNotFoundError(
            f"Missing {best_window_path}"
        )
...
def run_stage8(config, targets=None, target_glob=None):
    ...
    if not os.path.exists(windows_csv):
        raise FileNotFoundError(f"{windows_csv}")
    df_windows = pd.read_csv(windows_csv).sort_values("Rank").reset_index(drop=True)
    df_reference = load_reference(config.reference_csv)

    for mast in masts:
        if not os.path.exists(mast.best_window_path):
            print(f"missing {mast.best_window_path}")
            continue
        ...
        if speed_col_original not in df_original.columns:
            raise KeyError(f"{speed_col_original} in {mast.raw_path}")
        if speed_col_corrected not in df_corrected.columns:
            raise KeyError(f"{speed_col_corrected} in {mast.best_window_path}")
